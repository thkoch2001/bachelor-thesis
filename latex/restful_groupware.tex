...
% - present a restful architecture as a useful and advantageous alternative to
% more complex and not restful approaches like CalDAV, CardDAV and OpenSocial.

% - Beschreibung der Aufgabenstellung

%   - RESTful Groupware API
%   - wie können ganz verschiedene Medientypen unterstützt werden
%   - wiederverwendbare Komponenten
%   - betrachtung des Hypermedia supports, was muss Client wissen
%   - 
...
The software system designed in this work should provide an HTTP based interface
...
It should also be ensured, that the design can easily be extended to expose all
functionality via an HTML based interface to a regular browser.
...
The requirements list in section \ref{sec:scope-gener-requ} also lists an HTML
bases user interface. This subsection and the following discuss special design
issues concerning this requirement.
...
An HTML based user interface for a groupware today has many means to provide
data editing and submission facilities thanks to powerful Javascript libraries
like the VIE Editor (\autoref{sec:semantic-anno-use-cases}). The traditional,
...
A common way to produce HTML is by using a template engine. The jersey framework
provides a convenient integration mechanism for this purpose called ``implicit''
or ``explicit views'' which is unfortunately not documented in the user
guide\footnote{\citeurl{http://jersey.java.net/nonav/documentation/latest/user-guide.html}{2012-04-04}}
but only in a blog post by one of the developers \cite{Sandoz2008}.

Both mechanisms rely on a provisioned \lstinline:ViewProcessor: implementation
(Listing \ref{fig:templateprocessor}) that integrates an arbitrary template
engine. The \lstinline:Viewable: instance in the listing can either wrap an
instance of a jersey resource class in which case it is called an ``implicit
view'' or be explicitly returned by a resource handler method in which case the
developer provides an arbitrary object to be wrapped inside the
\lstinline:Viewable: as in Listing \ref{fig:viewable}.

\begin{javalisting}[label=fig:templateprocessor,
                   caption={Jersey interface to integrate template engines}]
...
\begin{javalisting}[label=fig:viewable,
                   caption={Jersey resource method explicitly returning a Viewable}]
...
The latter case is of interest for this work since the jersey resource classes
as designed in this implementation don't actually represent the resources and
their data. 

The combination of the above mechanism with the earlier presented resource
facades however is a bit more complicate. The resource facades mechanism on the
one hand provides objects of arbitrary type and the template engine integration
also accepts objects of arbitrary type. But at one point, a decision must be
made, which facade should be used and which template should be
rendered. Different strategies can be easily identified to solve this problem
...
% das Problem wird nur bedingt klar, vor allem wenn man die Notation in
...
A couple of interesting questions came up during research and implementation for
this work which could not be included. The following subsection lists some of
these questions, explain their relation to the scope of this work and provide
...
available commands for a specific MediaType via the \lstinline:getCommand:
method. The framework however was designed with the needs of a Desktop clipboard
in mind. Since JAF has been released for Java version 1.4 it also does neither
...
The resource facades concept shows similarities with Dependency Injection since
dependencies of a facade are also provided by an external component. It may be
possible that the concept could even be implemented on top of an existing
Dependency Injection framework.\footnote{Scala can provide Dependency Injection
  solely with language features via the so called Cake Pattern \cite{Warski2011}
  \cite{Odersky2005}.}  Some aspects however may require extra care:
...
The proposed resource facades implementation has the disadvantage that the
...
systems may be able to considerably improve the presented facades approach. 

\section{Conclusions}
\label{sec:conclusions}

% Check gegen Aufgabenstellung
