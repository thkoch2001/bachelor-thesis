...
\section{only draft below this section!}
...
\subsubsection{Client Design considerations}
...
% identifies the support of different media types to represent one resource "can complicate and
% hinder the interoperability" and "requires more maintenance effort".

% Different categories of media types:
% CSV, binary/plain text, large binary (video), tree (XML/JSON)
% Media types for human consumption have a greater variety in form, structure, content while media types for computer consumption may be more complex?
...
Activity Streams has been defined from the beginning as an XML (ATOM) and JSON format.\footnote{\citeurl{http://activitystrea.ms/}{2012-01-21}}
...
Responsibilites of a media type aware code:
\begin{itemize}
\item validate
\item provide accessors to read, write
...
Motivation für wiederverwendbare Komponenten im Zusammenhang mit Schreier: Möglichst wenig Code zu generieren, generierter Code benutzt wiederverwendbaren Code.
