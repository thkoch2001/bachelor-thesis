...
Although computers became ubiquitous for some time now, it is amazing that the
...
flexibility enables to reuse the API concepts for different types of clients
(\autoref{sec:user-class-char}) or content.
...
code'' and a special way to configure the involved components. On the server
...
Kolab is still developed, supported and distributed collaboratively by multiple
...
how Kolab uses IMAP, from the characteristics of the managed data set and the
...
help deciding whether a protocol should support locking. In the present case,
...
but both have not yet produced a release yet and appear abandoned since at least
2010.
...
not become complicate even while serving and accepting arbitrary Mediatypes.
...
observed that translate in the following advises for Mediatype libraries:
...
expected from it official status. A couple of smaller classes were written for
this work to wrap Jersey's functionality and that should be reusable in
arbitrary REST projects. Some of those have been presented in
\autoref{sec:prep-requ-comp}: Preconditions, PathParams or
PaginationRange. Additionally to this, a separate \lstinline:Routes: class
centralized the mapping from URI paths to resource classes, a LinkBuilder
centralized construction of URIs and the reader and writer provider classes
delegated their work to the resource facades framework. It seems advisable for
further REST projects to evaluate frameworks offering alternative concepts than
JAX-RS.
...
OpenSocial. It seems even possible to serve the use cases of both protocols,
...
Groupware API but for other applications as well that offer a restful HTTP API
and might consume or produce multiple Mediatypes.

A great source of simplification and serendipitous reuse (e.g., VCards for
social network information, \autoref{sec:vcards-soci-netw}) is caused by the
hypermedia support of the involved Mediatypes. The inclusion of complete,
dereferenceable URIs obsoletes the need for verbose API documentations (like
Opensocial, \autoref{sec:opensocial-background}. The absence of hypermedia
support, like missing links to groups or albums in vCards, can directly
constrain the possible use cases of a Mediatype.
...
expected. The initial research was spent to evaluate different platforms to
implement restful HTTP interactions. This area however proofed to be rather easy
and well supported compared to the immense shortcomings or even lack of
Mediatype libraries. This makes one wondering, whether an expert group like
